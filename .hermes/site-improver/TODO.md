...
C-06 was the last deferred item — it requires real press coverage the practice doesn't yet have. T-10 (2026-06-28) closed the last remaining internal-CTA tracking gap: condition cards (the top-of-funnel CTAs on the homepage and the entire content of /conditions/) were firing no dataLayer event, so GA4 funnels could see "user landed on /" but not "user clicked Sudden Hearing Loss from the homepage". With T-10, every link on the site — internal route, tel:, mailto:, external social/GBP, form submission, AND the conditions grid — fires a labeled dataLayer event to GTM. The TODO is now functionally exhausted of safe, non-fabricating items; any future high-impact work will come from new audit findings. Last shipped: T-10 (condition-card click tracking).
...
- [x] **T-10** — Condition-card click tracking (`/conditions/` and homepage grid) ✅ DONE 2026-06-28
  ...
  - The `cta_label` is the condition's display name (e.g., "Sudden Hearing Loss", "Diabetic Lower-Extremity Wounds"), giving GA4 per-condition attribution from the homepage grid. The `location: "conditions_grid"` label groups the homepage grid, the `/conditions/` page grid, and the related-conditions block on each condition detail page into one GA4 funnel step; `page_location` (auto-captured by GA4 page_view) joins the rest of the context.
  - Single-file change, 7 insertions(+), 0 deletions(-). No schema changes (tracking/HTML-only). The component was a server component; converting to `"use client"` is the only structural change, and it follows the exact convention every other instrumented component on the site already follows.
