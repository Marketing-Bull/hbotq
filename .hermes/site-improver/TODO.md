...
- [x] **CR-02** — ✅ DONE 2026-07-30 (automated nightly run, commit `29431e2`)
  - Converged this branch onto `main`'s array-form Review pattern (one `<script>` tag holding `testimonials.map(reviewSchema)`) rather than the reverse, since `main` is the live production branch and this was the smaller-footprint change. Updated both call sites: `app/page.tsx` and `app/condition/[slug]/page.tsx`.
  - Restored `reviewSchema()`'s own `"@id": \`${site.url}/#review-${t.id}\`` field (reverted the function to take a `Testimonial` directly instead of a destructured options object that had dropped it). Kept this branch's `itemReviewed.url` addition — a harmless additive enrichment `main` doesn't have.
  - Verified: `npm run build` passes; validator (fixed by CR-01) reports 0 errors/0 warnings with no Review-specific issues; confirmed `@id` present in real HTML output for all 5 reviews; confirmed 1 script tag holds all 5 entries (not 5 separate tags); structurally diffed this branch's homepage output against a fresh `main` build for the same page — identical key sets on every Review node, same array-of-5-in-one-script pattern, no divergence risk left on merge. eslint clean.

**Night 3 — CR-03:**
- [x] **CR-03** — ✅ DONE 2026-07-31 (automated nightly run, commit `4a09756`)
  - Fixed `components/analytics/scroll-depth.tsx`: `computePercent()` now returns 100 directly when `trackableHeight <= 0` (nothing to scroll) instead of clamping the denominator to 1, which had forced `percent` to compute as 0 forever on short pages.
  - Verified with a real headless-browser test (Playwright + the pre-installed Chromium), not just by reading the diff: built the unfixed code and loaded `/thank-you/` with a `dataLayer` listener and zero scrolling — 0 `scroll_depth` events, reproducing the bug. Built the fixed code, same test — all 4 thresholds (25/50/75/100) fire immediately on mount.
  - Regression-checked the scrollable-page path (homepage, small viewport) against both the unfixed and fixed builds — identical behavior on both, confirming the fix only added an early-return branch and didn't touch the existing scrollable-page logic. (Noted a pre-existing, unrelated quirk in that test where a single `scrollTo`-to-bottom call only captured the 25% threshold in headless Chromium — reproduced identically on both unfixed and fixed code, so it predates this change and isn't a regression; not in scope for CR-03.)
...
- [x] **T-09** — Footer outbound social/GBP link click tracking ✅ DONE 2026-06-27
  - Fresh audit (2026-06-27) found that `components/layout/footer.tsx` renders three outbound social links — Facebook, Instagram, Google Business Profile — as `<a>` tags with `target="_blank"` and `rel="noopener noreferrer"`, but with **no `onClick` dataLayer handler**. T-03 explicitly enumerated the footer in its scope ("Applied to all phone/email links across: ... Footer") but only wired the `tel:` and `mailto:` anchors (lines 38–51); the three social/profile anchors (lines 54–86) were missed. These were the last outbound-conversion paths on the site with no GTM event — every other internal link, tel:/mailto: link, and form submission was already instrumented by T-01 / T-02 / T-03 / T-06 / T-08.
  - The GBP click specifically is the highest-intent external action the footer exposes (drives Google review reads + directions), so not tracking it was a meaningful attribution gap.
  - Fix in `lib/analytics/track.ts`: extended `trackClick`'s `category` union from `"phone_call" | "mailto" | "cta_click"` to `"phone_call" | "mailto" | "external" | "cta_click"`. The runtime behavior is unchanged — `"external"` falls into the same `event: "outbound_click"` branch as `"phone_call"` and `"mailto"`, so any existing GTM triggers listening for `event === "outbound_click"` already cover these new clicks without a GTM config change. The `outbound_category: "external"` field lets GTM funnels split social/GBP clicks from phone/email clicks if needed.
  - Fix in `components/layout/footer.tsx`: added `onClick={trackClick("external", { location: "footer", outbound_target: "facebook" | "instagram" | "google_business_profile" })}` to each of the three social/profile anchors. The `outbound_target` metadata makes per-network reporting possible in GA4.
  - 2 files changed, 12 insertions(+), 3 deletions(-). No schema changes (tracking/HTML-only). The footer was already a client component (`"use client"`), so no further conversion was needed.
  - `npm run build`: passes (20 routes). `npm run validate:schema`: **105 blocks / 0 errors / 0 warnings** (unchanged — no schema changes). Verified compiled JS bundle contains `trackClick("external",{location:"footer",outbound_target:"facebook"})` (and the instagram / google_business_profile variants) in the footer chunk, confirming the dataLayer payloads are wired. Verified rendered HTML on `/`: the three anchors still emit the correct `href`, `target="_blank"`, `rel="noopener noreferrer"`, and `aria-label` (no attribute regression).
...
- [x] **T-10** — Condition-card click tracking (`/conditions/` and homepage grid) ✅ DONE 2026-06-28
  ...
  - With T-10, the condition cards — the most-clicked internal CTA on the homepage — fire a labeled `cta_click` dataLayer event. Combined with T-01 through T-09, every link on the site is now instrumented. No GTM config change required.
  - **PR**: https://github.com/Marketing-Bull/hbotq/pull/41 (rolled into the existing open PR for this branch — SKILL.md guidance: skip PR creation when one is already open for the branch)

- [x] **T-11** — Consultation form funnel tracking (`form_view` / `form_start` / `form_field_complete`) ✅ DONE 2026-07-01
  - The consultation form on `/contact-us/`, `/hyperbaric-therapy/`, and all six `/condition/[slug]/` pages only fired `form_submit` on success (T-02, 2026-05-29). Every other step in the conversion journey was invisible to GA4: GA could see "user submitted" but not "user saw the form" or "user started filling it out" or "which fields they completed before submitting". This was the most important funnel on the site — the form is the only path to a lead — and the missing upstream events made conversion-rate optimization impossible.
  - Three new dataLayer events close the funnel, all reusing the existing `trackEvent()` helper from `lib/analytics/track.ts`:
    - **`form_view`** — fires once when the form scrolls into view, via an `IntersectionObserver` with `threshold: 0.1`. Falls back to firing on mount if IO is unavailable. Observer is disconnected after the first hit so each page-view fires at most once.
    - **`form_start`** — fires once on the first `focusin` event inside the form (focusin bubbles, so a single form-level handler covers all 5 fields).
    - **`form_field_complete`** — fires once per tracked field (`name` / `phone` / `email` / `condition` / `message`), on `blur` in the capture phase, when the field has a non-empty trimmed value. Captures per-field drop-off.
  - The 5 user-fillable fields are tracked; `consent` checkbox, `source` hidden input, and `website` honeypot are excluded from the funnel-tracker to avoid inflating the funnel and to keep spam-bot keystrokes from polluting the data.
  - Fix in `components/forms/consultation-form.tsx`: added 3 `useEffect` blocks (one per event) plus a `TRACKED_FIELDS` allowlist. Added `formRef = useRef<HTMLFormElement>(null)` plus `startedFiredRef` / `completedFiredRef: Set<TrackedField>` for fire-once semantics. Imported `useEffect` and `useRef` from react. 128 insertions(+), 1 deletion(-).
  - `npm run build`: passes (20 routes, 0 TypeScript errors). `npm run validate:schema`: **105 blocks / 0 errors / 0 warnings** (unchanged — no schema changes). Verified compiled JS bundle (`.next/static/chunks/01j0kwyu1elo7.js`) contains all four event names: `form_view`, `form_start`, `form_field_complete`, `form_submit` — each exactly once. The minifier inlined the `TRACKED_FIELDS.includes(name)` check to `dg.includes(n)` on the compiled form, confirming the field-name allowlist survives the build. Verified the IntersectionObserver fallback path is in the bundle (`typeof IntersectionObserver === "undefined"`).
...
- [x] **T-14** — Footer nav column link tracking ✅ DONE 2026-07-09
  - Fresh codebase walk (2026-07-09) found that the three `FooterColumn` instances — Explore ×4, Conditions ×6, Visit ×4 (14 total internal nav links) — rendered as plain `<Link>` elements with zero `onClick` dataLayer events. The footer columns are the primary navigation fallback for users who scroll past the hero and sticky CTA; GA4 funnels had a blind spot on the full lower-funnel navigation path. T-09 (2026-06-27) had already wired footer outbound links (phone, email, Facebook, Instagram, GBP), but the internal columns were missed.
  - Fix in `components/layout/footer.tsx`: added `onClick={trackClick("cta_click", { location: "footer_nav", cta_label: l.label })}` to the `<Link>` inside `FooterColumn`. Footer was already `"use client"` and `trackClick` was already imported — the change is 4 lines. No visual change, no schema change, no new dependencies.
  - `npm run build`: passes (20 routes, 0 TS errors). `npm run validate:schema`: **105 blocks / 0 errors / 0 warnings** (unchanged). Bundle grep: `footer_nav",cta_label:e.label}` confirmed in `.next/static/chunks/0farb9r76vi4f.js`.
  - With T-14, every internal and external link on the site fires a labeled dataLayer event: header nav (T-06), footer columns (T-14), condition cards (T-10), hero CTAs (T-07), footer outbound (T-09), sticky CTA (T-01), 404 recovery (T-12). No GTM config change required.
  - **PR**: https://github.com/Marketing-Bull/hbotq/pull/41 (rolled into the existing open PR for this branch — SKILL.md guidance: skip PR creation when one is already open for the branch)

- [x] **T-13** — FAQ accordion expand/collapse tracking ✅ DONE 2026-07-05
  - Fresh audit (2026-07-05) found that `components/sections/faq-section.tsx` — the accordion rendered on `/faqs/` (4 sections, 17 rows total) and every `/condition/[slug]/` page (per-condition FAQ subset) — fired **zero dataLayer events** on user interaction. GA4 could see "user landed on a page with a FAQ section" but not "which questions users actually open", "how many they open per page", or "which categories get the most engagement". The FAQ accordion was the last major interactive UI on the site without instrumentation — T-01/T-03/T-06/T-08/T-09/T-10/T-11 already closed every link, scroll, form, and 404-recovery event on the site.
  - Two new dataLayer events close the gap, both via the existing `trackEvent()` helper from `lib/analytics/track.ts`:
    - **`faq_expand`** — fires on user click opening a row (one event per open, regardless of how the row was previously closed).
    - **`faq_collapse`** — fires on user click closing a previously-open row.
  - Event payload (both events): `{ event, faq_id, faq_category, faq_question, faq_section_heading, page }`. `faq_id` is the `Faq.id` (e.g. `soft-vs-hard-chamber`, `how-many-treatments`) for per-question attribution; `faq_category` is the `Faq.category` (`general`/`treatment`/`conditions`/`logistics`) for topic-area segmentation; `faq_question` is the user-visible text for DOM cross-reference; `faq_section_heading` is the FaqSection's `<h2>` heading (one of 4 on `/faqs/`, or the dynamic `${condition.shortName} — common questions` on condition pages) for per-section engagement; `page` is `usePathname()` matching the T-05/T-11 pattern.
  - **Default-open behavior preserved**: the first row in each FaqSection is open by default (`defaultOpen={idx === 0}`); no event fires on mount — only on user click. Matches the T-11 `form_start` / T-12 `not_found_view` mount-once-without-firing-on-render convention.
  - Fix in `components/sections/faq-section.tsx`: added `usePathname` import from `next/navigation`; added `trackEvent` import from `@/lib/analytics/track`; `FaqRow` now accepts a `sectionHeading` prop (forwarded from FaqSection's `heading` prop); `onClick` handler flips state then calls `trackEvent` with the matching expand/collapse event (React batches both calls, no double-render). 1 file changed, 22 insertions(+), 4 deletions(-). No schema changes (tracking/HTML-only — visible DOM unchanged: same button, same `aria-expanded`, same toggle behavior, same answer reveal).
  - `npm run build`: passes (20 routes, 0 TypeScript errors). `npm run validate:schema`: **105 blocks / 0 errors / 0 warnings** (unchanged — no schema changes). Verified compiled JS bundle (`.next/static/chunks/01g45tqol89so.js`) contains the full payload shape `trackEvent(t?'faq_expand':'faq_collapse',{faq_id:e.id,faq_category:e.category,faq_question:e.question,faq_section_heading:o,page:d||'/'})` — confirming the dataLayer payload is wired.
  - Closes the last major untracked interactive UI on the site. Combined with T-01 through T-12, every meaningful user interaction on the site now fires a labeled dataLayer event to GTM: scroll depth, link clicks (internal / tel / mailto / external / 404-recovery), form submission, form funnel, FAQ expand/collapse, and 404 page-view. No GTM config change required.
  - **PR**: https://github.com/Marketing-Bull/hbotq/pull/41 (rolled into the existing open PR for this branch — SKILL.md guidance: skip PR creation when one is already open for the branch)
...
- **S-12** — VideoObject deep validator in `scripts/validate-schema.mjs`. Previously all 21 VideoObject blocks across `/videos/` and 6 condition pages fired "no validator defined — skipping deep validation" warnings. Added `validateVideoObject()` function (142 lines) enforcing: required `name` + `description` (via `validateString`), required `thumbnailUrl` (string | string[] of http(s) URLs), required `uploadDate` (ISO 8601 date/datetime regex), recommended `embedUrl`/`contentUrl` (at least one; ERROR on malformed URLs), recommended `duration` (ISO 8601 PT#M#S; WARN when missing — data not yet in video data layer), recommended `publisher` (Organization object with `name` + `url`). Wired into `TYPE_VALIDATORS`. RED-GREEN test confirmed: removing `name` + `uploadDate` from a VideoObject block produces 2 errors; restoring returns to 0 errors. `npm run validate:schema`: **0 errors, 35 warnings** (21 `duration` warnings replacing 15 old "no validator defined" skipping warnings — net: 21 VideoObject blocks now deeply validated, 6 new duration warnings for condition-page embeds surface a data gap for future enrichment). 1 file changed, 142 insertions(+). (2026-07-12, PR #41 updated)
