...
**Cycle state (2026-07-11):** The original priority list (10 items: SE-01, SE-03, SE-04, SE-05, T-01, T-03, T-04, C-01, C-02, C-05) is **fully drained** — all are marked `[x]` in the checklists below with completion notes. S-11 (2026-07-11) fixed the 8 validator errors (array JSON-LD on condition pages) introduced by the main-branch expansion. All 40 items tracked in this file (S-01 → S-11, SE-01 → SE-07, T-01 → T-14, C-01 → C-05, WD-01 → WD-06) are shipped and rolled into PR #41. Validator now: 0 errors.

1. **S-11** — Fix per-Review JsonLd array emit on condition pages. ✅ DONE 2026-07-11 — see Schema category checklist. Last shipped: S-11.

**Future audit-driven candidates** (no `[ ]` items; these are leads for the next cycle's audit, not commitments):
- **VideoObject validator** — `videos.html` emits 18 VideoObject blocks but the validator skips deep validation for that type (35 warnings). Worth adding a `validateVideoObject()` rule to `scripts/validate-schema.mjs` to enforce `name`, `description`, `thumbnailUrl`, `uploadDate`, `embedUrl` per Google's Video structured data docs.
